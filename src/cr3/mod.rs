...
pub fn extract_thumb(file_path: &String, output: &String){
    ...
    let width = u16::from_be_bytes(raw_file[index+14..=index+15].try_into().unwrap());
    let height = u16::from_be_bytes(raw_file[index+16..=index+17].try_into().unwrap());
    let jpeg_size = u32::from_be_bytes(raw_file[index+20..=index+23].try_into().unwrap());
